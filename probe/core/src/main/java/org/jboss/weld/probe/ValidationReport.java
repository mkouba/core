...
public class ValidationReport {
    ...
    public ValidationReport(Probe probe) {
        this.probe = probe;
    }

    void exportValidationReport(Exception exception, Environment environment, BeanManagerImpl manager) {

        HtmlTag html = HtmlTag.html();
        HtmlTag head = html.addAndGet(HtmlTag.head());
        head.add(HtmlTag.title(TITLE));
        head.add(HtmlTag.style().add(IOUtils.getResourceAsString("/report.css")));

        HtmlTag body = html.addAndGet(HtmlTag.body());

        body.add(HtmlTag.h1(TITLE));
        HtmlTag meta = body.addAndGet(HtmlTag.stripedTable());
        ...
        HtmlTag contents = body.addAndGet(HtmlTag.ol());
        ...
        try {
            File exportFile = new File(exportPath, "weld-validation-report.html");
    ...
    private void addInvalidDependencies(HtmlTag body) {

        body.add(HtmlTag.aname(DEPS));
        body.add(HtmlTag.h2(TITLE_DEPS));

        HtmlTag table = body.addAndGet(HtmlTag.div(DEPS).addAndGet(HtmlTag.stripedTable()));
        table.addAndGet(HtmlTag.tr()).add(HtmlTag.th(""), HtmlTag.th("Declaring Bean"), HtmlTag.th("Problem"));
        int idx = 0;
        ...
        for (Bean<?> bean : probe.getBeans()) {
            ...
            if (injectionPoints != null && !injectionPoints.isEmpty()) {
                for (InjectionPoint injectionPoint : injectionPoints) {
                    ...
                    if (beans.isEmpty()) {
                        // Unsatisfied
                        table.addAndGet(HtmlTag.tr()).add(HtmlTag.td(++idx + "."),
                                HtmlTag.td(bean.toString()).add(HtmlTag.td("Unsatisfied dependency at " + injectionPoint)));
                        try {
                            beanManager.resolve(beans);
                        } catch (AmbiguousResolutionException e) {
                            // Ambiguous
                            table.addAndGet(HtmlTag.tr()).add(HtmlTag.td(++idx + "."),
                                    HtmlTag.td(bean.toString()).add(HtmlTag.td("Ambiguous dependency at " + injectionPoint)));
                        }
                    }
                }
            }
    ...
    private void addBeanArchives(HtmlTag body, List<BeanDeploymentArchive> bdas) {

        body.add(HtmlTag.aname(BDAS));
        body.add(HtmlTag.h2(TITLE_BDAS));

        HtmlTag table = body.addAndGet(HtmlTag.div(BDAS).addAndGet(HtmlTag.stripedTable()));
        table.addAndGet(HtmlTag.tr()).add(HtmlTag.th(""), HtmlTag.th("Identifier"), HtmlTag.th("Bean Discovery Mode"), HtmlTag.th("beans.xml"));
        int idx = 0;

        for (BeanDeploymentArchive bda : bdas) {
            HtmlTag tr = table.addAndGet(HtmlTag.tr()).add(HtmlTag.td(++idx + "."), HtmlTag.td(bda.getId()));
            ...
            if (beansXml != null && !beansXml.equals(BeansXml.EMPTY_BEANS_XML)) {
                tr.addAndGet(HtmlTag.td()).addAndGet(HtmlTag.div())
                        .attr(HtmlTag.TITLE, beansXml.getUrl() != null ? beansXml.getUrl().toString() : "URL not available")
                        .add(beansXml.getVersion() != null ? beansXml.getVersion() : "Version not defined");
            } else {
    ...
    private void addConfiguration(HtmlTag body, BeanManagerImpl manager) {

        body.add(HtmlTag.aname(CONFIG));
        body.add(HtmlTag.h2(TITLE_CONFIG));

        HtmlTag table = body.addAndGet(HtmlTag.div(CONFIG).addAndGet(HtmlTag.table()));
        table.addAndGet(HtmlTag.tr()).add(HtmlTag.th(""), HtmlTag.th("Key"), HtmlTag.th("Default Value"), HtmlTag.th("Value"), HtmlTag.th("Description"));
        int idx = 0;
        ...
        for (ConfigurationKey key : getSortedConfigurationKeys()) {
            ...
            if (!defaultValue.equals(value)) {
                ...
                table.add(HtmlTag.tr().add(HtmlTag.td(++idx + "."), HtmlTag.td(key.get()), HtmlTag.td(defaultValue.toString()), HtmlTag.td(value.toString()),
                        HtmlTag.td(desc != null ? desc : "")));
            }
        }
    }

    private void addBeans(HtmlTag body, List<BeanDeploymentArchive> bdas) {

        body.add(HtmlTag.aname(BEANS));
        body.add(HtmlTag.h2(TITLE_BEANS));

        HtmlTag table = body.addAndGet(HtmlTag.div(CONFIG).addAndGet(HtmlTag.stripedTable()));
        table.addAndGet(HtmlTag.tr()).add(HtmlTag.th(""), HtmlTag.th("Archive"), HtmlTag.th("Kind"), HtmlTag.th("Bean Class"), HtmlTag.th("Types"),
                HtmlTag.th("Qualifiers"), HtmlTag.th("Scope"));
        int idx = 0;
                ...
                if (bdaId.equals(beanManager.getId())) {
                    ...
                    for (Iterator<Type> iterator = JsonObjects.sortTypes(bean.getTypes()).iterator(); iterator.hasNext();) {
                        ...
                        if (!Object.class.equals(type)) {
                            types.append(HtmlTag.div().attr(HtmlTag.TITLE, formatted).add(abbreviateType(formatted)));
                            if (iterator.hasNext()) {
                                types.append(HtmlTag.BR);
                            }
                        }
                    }
                    ...
                    if (bean.getQualifiers() != null && !bean.getQualifiers().isEmpty()) {
                        for (Iterator<Annotation> iterator = bean.getQualifiers().iterator(); iterator.hasNext();) {
                            ...
                            if (Any.class.equals(qualifier.annotationType())) {
                                ...
                                qualifiers.append(JsonObjects.simplifiedAnnotation(qualifier));
                            } else {
                                qualifiers.append(HtmlTag.div().attr(HtmlTag.TITLE, qualifier.toString()).add(abbreviateAnnotation(qualifier.toString())));
                            }
                            if (iterator.hasNext()) {
                                qualifiers.append(HtmlTag.BR);
                            }
                        }
                    }
                    String beanClass = Formats.formatType(bean.getBeanClass(), false);
                    table.addAndGet(HtmlTag.tr()).add(HtmlTag.td(++idx + "."), HtmlTag.td().add(HtmlTag.a("#" + BDAS).add(" " + (i + 1))),
                            HtmlTag.td(BeanKind.from(bean).toString()),
                            HtmlTag.td().add(HtmlTag.div().attr(HtmlTag.TITLE, beanClass).add(abbreviateType(beanClass))), HtmlTag.td(types.toString()),
                            HtmlTag.td(qualifiers.toString()), HtmlTag.td(JsonObjects.simplifiedScope(bean.getScope())));
                }
            }
        }
