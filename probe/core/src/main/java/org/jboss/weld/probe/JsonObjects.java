...
final class JsonObjects {
    ...
    static String createDeploymentJson(BeanManagerImpl beanManager, Probe probe) {
        ...
        for (ConfigurationKey key : ValidationReport.getSortedConfigurationKeys()) {
            Object defaultValue = key.getDefaultValue();
            String desc = ValidationReport.getDesc(key);
            if (desc == null) {
                // Don't show config options without description
                continue;
            }
            Object value = ValidationReport.getValue(key, configuration);
            if (value == null) {
